...
- **Entrada Inteligente**: O usuário digita de forma livre os ingredientes disponíveis na geladeira/despensa (ex: _"tenho 3 ovos, um punhado de espinafre e metade de uma cebola"_). A IA interpreta, extrai e estrutura o inventário automaticamente.
...
A interface segue uma abordagem _mobile-first_, adaptada perfeitamente a desktop de modo limpo e espaçoso. As principais telas e seus fluxos incluem:
...
3. **INT-003 - Tela da Despensa Inteligente (Dashboard):**
   - _Coluna Esquerda (2/3):_ Campo de entrada de texto livre (`TextArea`) para processar ingredientes.
   - _Coluna Direita (1/3):_ Lista reativa de ingredientes estruturados com controle de remoção individual (ícone de lixeira vermelha).
...
### 5. Testes E2E com Playwright + Clerk (Opcional)

Configure e execute testes automatizados de ponta a ponta:

```bash
# Executar setup rápido
./setup-playwright.sh

# Editar configuração do Clerk
nano apps/frontend/.env.test

# Rodar testes (em outro terminal, se estiver usando vercel dev)
cd apps/frontend
npm run test:e2e
...
- 📖 [Guia Completo de Testes](apps/frontend/tests/docs/playwright-testing.md)
- 🎯 [Boas Práticas](apps/frontend/tests/docs/playwright-best-practices.md)
- ✅ [Checklist de Setup](playwright-setup-checklist.md)

### 📋 Planos de Testes Implementados

O projeto conta com planos de testes abrangentes para validar funcionalidades críticas:

#### **📝 Testes de Autenticação com Clerk (INT-001)**

- **Documento Principal:** [test-plan-clerk-authentication.md](test-plan-clerk-authentication.md)
- **Resumo Executivo:** [test-plan-summary.md](test-plan-summary.md)
- **Seção de Agente:** [.agents/sections/clerk-authentication-testing.md](.agents/sections/clerk-authentication-testing.md)
- **Prompts para Geração:** [prompts-test-generation.md](prompts-test-generation.md)

**Cobertura:** 10 cenários de teste incluindo:

- ✅ Login com credenciais válidas
- ✅ Validação de erros (email/senha inválidos)
...
- **Isolamento de Dados:** O sistema utiliza o padrão _Shared Database, Shared Process_ (Single-Tenancy Arquitetural com isolamento lógico por linha). Cada linha das tabelas (`PantryItems`, `UserProfiles`, `SavedRecipes`) é vinculada a um `UserId` exclusivo (do tipo `String` ou `UUID` fornecido pelo Clerk).
